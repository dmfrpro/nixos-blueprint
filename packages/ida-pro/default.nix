...
let
  pythonForIDA = pkgs.python314.withPackages (ps: with ps; [ rpyc ]);
...
pkgs.stdenv.mkDerivation rec {
  ...
  postFixup = ''
    cd $out/opt && ${pkgs.python314}/bin/python '${keygen}'
    cd $out/opt && ${pkgs.python314}/bin/python '${keygen_lumina}'
    cd $out/opt && ${pkgs.python314}/bin/python '${keygen_vault}'
  '';
